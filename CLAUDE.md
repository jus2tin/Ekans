...
- All of `Foldable`'s actual operations (`foldr`/`foldl`/`foldMap`/`fold`/`foldr1`/`foldl1`/`fold1`, `toList`, `length`, `sum`, and the rest of the pure-fold slice of `Data.Foldable` — see `docs/specs/foldable.md`) are free functions taking anything satisfying the protocol, not methods declared on it — the protocol itself declares nothing but `__iter__`. Every one of those functions is built on top of `__iter__` alone.
- **Correction, found during `docs/specs/foldable.md`'s Phase 1, applied here directly:** this section originally described `foldr`'s stack-safety mechanism as "bounce a thunk through an explicit loop instead of actually recursing" — a literal chain of nested closures, each deferring to the next via a call. That doesn't work: calling the outermost closure still recurses through a real Python stack frame per element, verified directly (a deliberately lowered recursion limit reproduces a genuine `RecursionError` on a 100,000-element input with that approach). The actual, verified-working mechanism has nothing to do with thunk-chaining: a plain accumulator loop over `reversed(list(xs))`, which needs `O(n)` auxiliary state (any right fold with a strict combining function does) but keeps that state in an ordinary Python list rather than on the call stack — zero recursion, verified clean at 100,000 elements even with the recursion limit artificially lowered. `foldl` needs no such care at all; a left fold's accumulator loop runs forward with no reversal, trivially stack-safe on its own.
- Alongside the Protocol, there's also an optional ABC, `FoldableABC`, that a concrete type can inherit from to override `foldr` or `length` with something more efficient than the generic `__iter__`-driven default (e.g. a type with O(1) access to its length shouldn't have to fold to compute one) — scoped to exactly those two override points, not one per derived function, since everything else in the module is built on `foldr` and inherits any improvement to it automatically. Free functions check for that override first — `isinstance(x, FoldableABC)` and catching the sentinel `NotImplementedError` the un-overridden default raises — and fall back to the generic implementation otherwise. This mirrors Haskell's own `Foldable`: a minimal-complete-definition method (`foldr`) with default implementations for everything else, where individual instances are free to override any of them. The point of keeping both: a plain `list` (which will never inherit anything) still gets every operation for free through the Protocol + defaults, while a type that legitimately benefits from a custom implementation isn't stuck with the generic one.
...
- `Compose[W, A]` (`newtype Compose f g a = Compose (f (g a))`) — one functor nested inside another. Added as a prerequisite for `Traversable`'s composition law, which needs a real `Compose` to state and test against. No HKT in Python, so `W` stands for the whole concrete nested type (e.g. `Just[Identity[A]]`) rather than tracking the outer/inner functors as separate type parameters; per-pair precision comes from free-function `@overload`s, same mechanism `fmap`/`ap` already use. `Functor`/`Foldable` this round; `Applicative` for the subset of pairs where both sides are nominally `Applicative`; `Traversable` deferred to `Traversable`'s own round (circular otherwise — see `docs/specs/compose.md`).
...
- `src/ekans/__init__.py` re-exports the full public API (every type-class ABC, every concrete type, every free function) via a curated `__all__`, so `from ekans import Identity` works alongside the existing fully-qualified `from ekans.identity import Identity` style used throughout `docs/HOWTO.md`.
- **Exception:** seven free functions — `ap`, `bind`, `do`, `all`, `sum`, `product`, `const` — are deliberately left out of that top-level re-export, because each shares a bare name with an existing submodule (`ap.py`, `bind.py`, `do.py`, `all.py`, `sum.py`, `product.py`, `const.py`). Re-exporting them bare would make e.g. `ekans.sum` resolve to either the free function or the submodule depending on unrelated import order elsewhere in a program, since Python rebinds a package attribute to a submodule every time that submodule is imported anywhere — verified directly rather than assumed. They stay reachable only via their fully-qualified path.
- **Discussed and deferred (2026-09-03):** the naming collision above could be eliminated outright by prefixing every submodule with `_` (e.g. `sum.py` → `_sum.py`), which would make each module's dotted path textually distinct from any re-exported name and let all seven functions move to the top level too. Decided against it for now: done properly it means renaming all ~23 files, not just the 7 colliding ones (a partial rename would be arbitrary rather than principled), which touches every internal cross-import in `src/ekans/` *and* every one of the ~40 fully-qualified import examples in `docs/HOWTO.md` — a real reversal of that guide's deliberate "each example shows exactly which submodule a name lives in" teaching design, for what's ultimately a cosmetic gain (seven names staying qualified-only costs little today). Revisit this once the package has genuinely outgrown its current small size — e.g. if the flat module count or the list of colliding names grows enough that the exception list itself becomes the awkward part, rather than the rename.
...
**Standing rule — the Compositional Invariance Matrix.** The type hierarchy must stay a mathematically self-consistent whole, not a pile of siloed features bolted on independently. Three obligations apply to every future round, enforced concretely in Phase 1 and Phase 4 below:
- **The Cross-Product Rule.** Whenever a new type class or concrete type is introduced, audit its interaction with every type class *compatible* with it — meaning any existing type class already sharing at least one concrete instance with the new capability's own instances (e.g. `Extractable` and `Pointed` are compatible because `Identity` implements both; `Sum` and `Pointed` are not, since nothing makes `Sum` `Pointed`). For each compatible pair, work out whether a genuine law connects them — the way `extract(point(a)) == a` connects `Extractable` to `Pointed` — and either write a property-based test for it or record, in the spec, exactly why no such law holds. No capability ships tested only against itself.
- **Cumulative Regression.** Every ticket's full check suite runs against the *entire* existing test suite, not just that ticket's new tests — every foundational law from every previous round must still pass. A new round is never allowed to silently weaken or break an earlier one.
- **The Proof Burden.** Any concrete type excluded from a new type class's scope needs a structural justification written into the spec (e.g. `Reader`/`Star` excluded from `Extractable` because they wrap a function, not a single value; `Proxy` excluded because it holds nothing at runtime) — never a silent omission or a hand-wave.
...
- **Cross-Product audit (Invariance Matrix, mandatory).** List every type class compatible with this round's new capability, per the Compositional Invariance Matrix rule above. For each one, work out by hand whether a genuine law connects it to the new capability, and verify any that do hold (same rigor as any other law in this phase — hand-derive the formula, check it against a correct implementation, check it's caught by a deliberately broken one). A pair with no meaningful law is a legitimate outcome, but it must be reasoned through and written down, not skipped because nothing obvious came to mind.

**Phase 2 — Draft the spec, wait for approval.**
`docs/specs/<name>.md` (see `docs/specs/functor.md` for the template shape: Status, Tickets link, Summary, Motivation, Design, Concrete instances in scope, Testing strategy, Documentation requirements, Implementation constraints, Out of scope, Open questions/risks). Every Design claim states what Phase 1 actually verified — "Verified against `mypy --strict`: ..." — not just what's intended. Every spec's Implementation constraints section includes, at minimum: "Implement only what is explicitly requested in the ticket. Do not add convenience functions, helper utilities, or alternative syntax sugar unless specified." The Testing strategy section carries the results of Phase 1's Cross-Product audit — which compatible type classes were checked, which laws were found and will be tested, and which pairs have no meaningful law and why. The Design or Out of scope section carries the Proof Burden — a structural justification for every concrete type excluded from this round's new class, not a silent omission. `Status: Draft — awaiting review` until approved, then flip to `Approved` in the same PR that gets merged. Nothing gets built against an unreviewed spec.
...
2. **Show the red step.** Write the failing test(s) first — including any cross-class law tests identified by Phase 1's Cross-Product audit, not just the ticket's own capability — run them, and show the actual pytest output (a real failure or a collection/import error) before writing any implementation code. Never claim or assume a test is red without having run it.
3. **Implement exactly what was approved.** No unrequested convenience functions, helper utilities, or alternative syntax sugar.
4. **Run the full check suite against the entire project, not just this ticket's new files:** `pytest` (100% coverage, zero regressions against every prior round's laws — the Cumulative Regression obligation), `mypy src tests --strict`, `black --check`, `isort --check`, `flake8`.
...
8. **Confirm the Proof Burden is met.** Every concrete type excluded from this ticket's type class has its structural justification written into the spec — not just implied by omission. If a genuine cross-class law surfaced in Phase 1 but wasn't yet reflected in the spec's Design section, correct the spec in place (same mechanism as step 6).
9. **Update `docs/HOWTO.md` with a real section or addition, not a stub.** If no concrete type implements the new class yet, use a small local illustrative type (matching the existing `Box` pattern) rather than one that doesn't actually support it yet. Verify every example in the doc actually runs before committing it — don't assume.
10. **Close the ticket** (`**Status:** Closed` in `TICKETS.md`) once its Definition of Done is verifiably met — no separate sign-off step required.
11. **Commit with a message that explains what shipped and any corrections/gotchas found along the way**, not just "implements X."
