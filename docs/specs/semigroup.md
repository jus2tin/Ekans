...
**Correction found during T-022, applied to T-022's file:** mypy rejects a single-variant `@overload` set outright (`error: Single overload definition, multiple required  [misc]`) — verified directly. So the free function can't start life as an `@overload` the way this section originally implied; it ships as a single plain-typed function against `Identity[S]` alone (same shape `fmap`'s T-001 and `ap`'s T-012 used for the same reason), and only becomes a real `@overload` set once T-023 adds `Const`'s second variant. Verified the plain-typed version is exactly as precise as the intended overload (`reveal_type` on `mappend(Identity[Box], Identity[Box])` correctly gives `Identity[Box]`) and still genuinely rejects `Identity[str]` with the same `[type-var]` error shown above.

**Correction found during T-023, applied to T-023's file:** the `@overload` implementation body can't just delegate to a shared method the way `ap`'s does (`return x.ap(f)`) — `Identity` and `Const` don't nominally implement `Semigroup`, so neither has a `.mappend()` method to call on itself, only their held value does, and the two containers construct differently (`Identity(value=...)` vs. `Const(value=...)`). The implementation body instead dispatches on `isinstance`, typed `(a: Union[Identity[S], Const[S, A]], b: Union[Identity[S], Const[S, A]]) -> Union[Identity[S], Const[S, A]]` — a real, precise Union rather than `Any`, per project review (the whole point of this library is not reaching for `Any` in the engine room). Verified this type-checks cleanly under `mypy --strict`: `isinstance` narrows `a`/`b` correctly inside each branch, and — because only the `@overload` *signatures* are checked at call sites, never the implementation's own signature — both overloads still resolve with full precision (`reveal_type` confirms `Identity[Box]` and `Const[Box, str]` exactly) and `Const[str, int]` is still genuinely rejected.
