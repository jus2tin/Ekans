...
**Correction found during T-048, applied to T-048's file:** verified via `reveal_type` that `bind(f, x)` silently degrades to `Any` -- not the loose `Bind[B]`, `Any` -- when `f` is passed as a bare, unannotated lambda (e.g. `bind(lambda a: Identity(value=str(a)), x)`). The free function's `f`-first argument order means mypy has to type-check the lambda before it has resolved `x`'s type to pick the right `@overload` branch; without an explicit parameter annotation on the lambda, that inference fails silently rather than erroring, and the whole call falls back to `Any`. Verified this is specific to the free function: the equivalent method call, `x.bind(lambda a: Identity(value=str(a)))`, infers precisely as `Identity[str]` with no issue, since `self` already anchors the type before the lambda is processed. Verified the free function still works precisely when `f` is a properly-typed `def` instead of a bare lambda (`bind(f, x)` where `f` has an explicit signature resolves to `Identity[str]` exactly). Not fixed by changing the established `f`-first convention (already approved in Phase 0, matches `fmap`/`ap`) -- documented as a real, known precision gap instead: prefer the method form (`x.bind(f)`) over the free function when passing a bare lambda inline.
