...
**Correction found during T-051, applied to T-050's file:** the "no re-declaration needed" finding above was verified against a *concrete* subclass's own chain (`Box.bind(f).bind(g)` where `Box` already narrows `bind` to `Box[B]`) -- but the law helper's `point` parameter is typed against the *abstract* `Monad[A]` handle, not a concrete type. Verified directly: calling `.bind()` on a value statically typed as `Monad[A]` (with no concrete subclass in sight) resolves to the inherited, un-narrowed `Bind.bind` signature, revealing `Bind[B]`, not `Monad[B]` -- exactly the same gap `Applicative` re-declaring `fmap`/`ap` from `Apply` already exists to close, for the identical reason: a generic law helper needs the *abstract* handle to stay narrowed, which only the class itself re-declaring the method (not a concrete subclass overriding it later) can provide. Fixed by adding `Monad`'s own `bind` re-declaration (narrowing `Bind[B]` to `Monad[B]`, `# type: ignore[override]` on the parameter, same treatment `Applicative.ap` already needed). Verified the fix resolves the law helper's chain precisely (`Monad[B]`) and doesn't disturb `Identity`/`Reader`'s own already-narrower overrides.
