...
**Correction found during T-033, applied to T-031's file:** flagged in review -- a type implementing *both* `Pointed` and `Extractable` has a real, testable law connecting them: `extract(point(a)) == a` (`extract . point == id`). This isn't a law of `Extractable` alone (mirroring how `Pointed` alone has no law either) -- it's a property of the *combination*, the same way `Applicative`'s four laws are properties of `Pointed` + `Apply` together, not of either piece alone. Of the six types in this round, only `Identity` implements both `Pointed` and `Extractable` (`Sum`/`Product`/`All`/`Const`/`Ap` are `Extractable` but not `Pointed`), so this is added as a single Hypothesis-checked test directly in `tests/test_identity.py` rather than a new reusable law-helper module -- following the same "wait for a second concrete need before generalizing" precedent as the free-function `@overload` growth pattern (`fmap`'s T-001, `ap`'s T-012, `mappend`'s T-022). Verified the law holds via Hypothesis before writing this down. If a second type ever implements both, this is the natural point to extract a shared `assert_point_extract_law` helper.
