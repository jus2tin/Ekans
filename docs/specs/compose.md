...
- **Correction found during T-075's implementation, applied there directly.** `W` is invariant (no `covariant=True` on its `TypeVar`), so a naturally-constructed value — `Compose(value=Just(value=Identity(value=1)))` — infers as `Compose[Just[Identity[int]], int]`, which doesn't structurally match a `Maybe[A]`-general overload at all (invariant generics need an *exact* match, not a subtype). 16 of the 25 pairs involve `Maybe` and/or `Either` in some position, and for all 16, the general-union overload was silently ineffective for the single most common way to actually build one — verified directly, then fixed by adding 16 more overloads using `Just`/`Right` (the "populated" variant) in place of `Maybe`/`Either`, above the general-union ones so mypy tries the more specific match first, mirroring `functor.py`'s own pre-existing `Just`/`Nothing`/`Maybe` and `Left`/`Right`/`Either` three-way pattern. `Nothing`/`Left`-specific overloads are deliberately *not* added — their `fmap` is a pure re-tag no-op, so the precision loss on the empty branch is lower-stakes than on the populated one, and the general `Maybe`/`Either` overload still covers them (just without full precision unless the caller widens the annotation). 41 total `fmap` overloads for `Compose`, not 25.
